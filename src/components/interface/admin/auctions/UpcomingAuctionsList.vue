<template>
  <div
    data-cy="admin-upcoming-auction-list-table"
    class="pr-10"
  >
    <table class="cursor-default w-full border border-unicornSilver rounded">
      ...
      <tbody
        v-for="(auction, i) in state.auctions"
        :key="i"
      ...
      >
        <tr class="h-12 font-source text-sm pl-4">
          ...
          <td>
            {{ auction.startDate.toDateString().substr(4, 3) }}
            {{ auction.startDate.getDate() }}
            {{ auction.startDate.getFullYear() }}
          </td>
          <td>
            {{ auction.endDate.toDateString().substr(4, 3) }}
            {{ auction.endDate.getDate() }}
            {{ auction.endDate.getFullYear() }}
          </td>
          <td>
            <div>
              <img
