  ...
  >
    ...
    >
      <tr
        ...
        @click="handleRedirect(lot.lotId)"
      >
        <td class="pl-4">
          <div> 
            {{ parseInt(lot.lotId) }}
...
async function handleRedirect(lotNumber: string | number) {
  await router.push({ name: "admin.barrel-info", params: { lotNumber: lotNumber } })
}
</script>
